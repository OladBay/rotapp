...
function Calendar() {
  ...
  const workedDays = myShifts.filter(s => !cancelledShifts.includes(s.date)).length
      ...
      <div style={s.body}>
        ...
        <div style={s.weekGrid}>
          {WEEK.map(({ date, day, label }) => {
            ...
            const isCancelled = cancelledShifts.includes(date) &&
            ...
            return (
              ...
              >
                ...
                {isCancelled ? (
                  ...
                  <div style={{
                    ...s.shiftCard,
                    background: shift.type === 'early'
                      ? 'rgba(42,127,98,0.15)'
                      : 'rgba(122,79,168,0.15)',
                    border: shift.type === 'early'
        ...
        {/* Shift detail modal */}
        {selectedShift && (
          <div style={s.overlay} onClick={() => { setSelectedShift(null); setCancelConfirm(false) }}>
            <div style={s.modal} onClick={e => e.stopPropagation()}>

              <div style={s.modalHeader}>
                <div style={s.modalTitle}>Shift Details</div>
                <button style={s.closeBtn} onClick={() => { setSelectedShift(null); setCancelConfirm(false) }}>✕</button>
              </div>

              <div style={s.modalBody}>
                <div style={s.detailRow}>
                  <span style={s.detailLabel}>Day</span>
                  <span style={s.detailVal}>{selectedShift.day}, {selectedShift.date}</span>
                </div>
                <div style={s.detailRow}>
                  <span style={s.detailLabel}>Shift</span>
                  <span style={{
                    ...s.detailVal,
                    color: selectedShift.type === 'early' ? '#2a7f62' : '#7a4fa8',
                ...
                <div style={s.detailRow}>
                  ...
                  <span style={{...s.detailVal, color:'#2ecc8a'}}>✓ Confirmed</span>
                </div>
              </div>
              ...
              {!cancelConfirm ? (
                ...
                <div style={s.confirmBox}>
                  <p style={s.confirmText}>
                    Are you sure? Your manager will be notified and this shift will need to be filled.
                  </p>
                  <div style={s.confirmActions}>
                    <button style={s.confirmNo} onClick={() => setCancelConfirm(false)}>
                      Keep shift
                    </button>
                    <button style={s.confirmYes} onClick={() => handleCancel(selectedShift)}>
                      Yes, cancel it
                    </button>
                  </div>
                </div>
...
const s = {
  ...
  stat: { background: '#161820', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '10px', padding: '12px 20px', textAlign: 'center' },
  ...
  dayCard: { background: '#161820', borderRadius: '12px', padding: '14px', minHeight: '120px', display: 'flex', flexDirection: 'column', gap: '10px', transition: 'border-color 0.15s' },
  dayTop: { display: 'flex', flexDirection: 'column', gap: '2px' },
  dayName: { fontSize: '11px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' },
  dayDate: { fontSize: '15px', fontWeight: 600, fontFamily: 'Syne, sans-serif' },
  shiftCard: { borderRadius: '8px', padding: '8px', flex: 1 },
  shiftType: { fontSize: '12px', fontWeight: 600 },
  shiftTime: { fontSize: '11px', color: '#9499b0', marginTop: '3px', fontFamily: 'DM Mono, monospace' },
  shiftHome: { fontSize: '11px', color: '#5d6180', marginTop: '4px' },
  offDay: { fontSize: '12px', color: '#5d6180', marginTop: 'auto' },
  cancelledTag: { fontSize: '11px', color: '#e85c3d', background: 'rgba(232,92,61,0.1)', borderRadius: '6px', padding: '4px 8px', width: 'fit-content' },
  ...
  cancelShiftBtn: { width: '100%', background: 'rgba(232,92,61,0.1)', border: '1px solid rgba(232,92,61,0.25)', color: '#e85c3d', padding: '12px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', borderTop: '1px solid rgba(255,255,255,0.07)' },
  confirmBox: { padding: '16px 24px', borderTop: '1px solid rgba(255,255,255,0.07)' },
  confirmText: { fontSize: '13px', color: '#9499b0', marginBottom: '12px', lineHeight: 1.5 },
  confirmActions: { display: 'flex', gap: '8px' },
  confirmNo: { flex: 1, background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#9499b0', padding: '9px', fontSize: '13px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' },
  confirmYes: { flex: 1, background: 'rgba(232,92,61,0.15)', border: '1px solid rgba(232,92,61,0.3)', borderRadius: '8px', color: '#e85c3d', padding: '9px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' },
}
